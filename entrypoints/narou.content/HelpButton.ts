...
const prepareHelpButtonForPc = (option: HelpButtonOption): void => {
  const hide = (): void => {
  ...
  const button = $(
    `<button class="tategaki-ni-narou-icon" title="縦書きになろうヘルプ・設定">${label}<img src="${browser.runtime.getURL("/icon/48.png")}"></button>`
  )
